...
namespace Cake.SqlPackage.UnitTests
{
    public sealed class SqlPackageScriptRunnerTests
    {
        ...
        {
            private readonly SqlPackageScriptFixture _fixture;

            public TheRunMethod()
            {
                _fixture = new SqlPackageScriptFixture();
            }
            ...
            public void Should_Still_Add_TargetDatabaseName_If_Target_File_Provided()
            {
                // Given
                _fixture.Settings.TargetFile = "./sqlpublishprofile.pubxml";
                _fixture.Settings.TargetDatabaseName = "mongoose";

                // When
                var result = _fixture.Run();

                // Then
                Assert.Contains("/TargetDatabaseName:mongoose", result.Args, StringComparison.OrdinalIgnoreCase);
            }
            ...
            {
                // Given
                _fixture.Settings.Variables.Add(key, value);
            ...
            public void Should_Add_Variables_After_Settings_If_Provided()
            {
                // Given
                _fixture.Settings.OverwriteFiles = true;
                _fixture.Settings.Variables.Add("CommandTimeout", "120");
